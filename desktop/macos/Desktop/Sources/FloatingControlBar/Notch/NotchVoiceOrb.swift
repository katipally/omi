...
struct NotchVoiceOrb: View {
  enum Mode: Equatable {
    case listening, thinking, speaking, logo

    /// The phase the lobe should show, or nil when no voice turn is on screen
    /// and the lobe belongs to the agent pills again. Order is a priority:
    /// live mic input wins over playback, which wins over the silent
    /// committed-but-not-yet-speaking stretch of the turn.
    static func current(listening: Bool, speaking: Bool, thinking: Bool) -> Mode? {
