...
struct DashboardPage: View {
  ...
  private func applyHomeStageObservers<Content: View>(to content: Content) -> some View {
    content
      ...
      // The landing keeps its centered composer on focus — only sending or a
      // deliberate scroll transitions to chat — so it is excluded here.
  ...
  ///
  /// The `homeMode == .hub` guard is what keeps restored history from tearing
  /// the landing away underneath the user: history restore, pagination and
  /// activate-refresh all churn the message count, and none of them are a
  /// user-facing "open the transcript". The landing is left only by an
  /// explicit action — sending, scrolling, or "Continue in Omi".
